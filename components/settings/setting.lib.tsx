...
export default function Setting() {
  ...
  return (
    ...
    >
      ...
      {open === "open" && (
        <div className="absolute right-0 bottom-13 min-w-60 flex-col items-end justify-center divide-y-2 divide-(--clr-muted-alt) rounded-lg border border-(--clr-muted-alt) bg-(--clr-bg) shadow-lg">
          {content === "" ? (
            <>
              <SettingInfo />
              <SettingButton
  ...
  function SettingButton({
    ...
    return (
      <button
        ...
        className={`flex w-full cursor-pointer items-center justify-between border-(--clr-muted) bg-(--clr-bg) px-4 py-3 text-sm text-(--clr-text) transition-colors duration-300 ease-in-out first:rounded-t-lg last:rounded-b-lg hover:bg-(--clr-muted) ${css}`}
      >
        {icon}
        {label}
      </button>
